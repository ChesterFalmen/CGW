...
window.onkeydown = (e) => {
    if (e.keyCode == 87) {
        p[0] = Math.min(p[0] + 0.01, 1);
    }
    else if (e.keyCode == 83) {
        p[0] = Math.max(p[0] - 0.01, 0);
    }
    else if (e.keyCode == 65) {
        p[1] = Math.max(p[1] - 0.01, 0);
    }
    else if (e.keyCode == 68) {
        p[1] = Math.min(p[1] + 0.01, 1);
    }
    console.log(p)
    draw()
}
...
function draw() {
    ...
    gl.uniform1f(shProgram.iS, parseFloat(document.getElementById("paramS").value));
    gl.uniform2fv(shProgram.iT, p);
    const v = equations(
        map(p[0], 0, 1, 0.25, parseFloat(document.getElementById("paramR").value)),
        map(p[1], 0, 1, 0, 2 * Math.PI),
    );
    gl.uniform3fv(shProgram.iCP, [v.x, v.y, v.z]);
...
function updateSurface() {
    ...
    scaleCoef = parseFloat(document.getElementById("paramS").value);
    document.getElementById("currentS").textContent = scaleCoef.toFixed(2);
...
function initGL() {
    ...
    shProgram.iS = gl.getUniformLocation(prog, "scaleCoef");
    shProgram.iT = gl.getUniformLocation(prog, "translate");
    shProgram.iCP = gl.getUniformLocation(prog, "currentPos");
